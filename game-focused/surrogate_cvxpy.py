...
def getDefUtility(single_data, T, s, unbiased_probs_pred, path_model, cut_size, omega=4, verbose=False, initial_coverage_prob=None, training_mode=True, training_method='two-stage', block_selection='coverage'):
    ...
    if training_mode and pred_optimal_res['success']:
        ...
        try:
            ...
            A_default, b_default = cp.Parameter((1, variable_size)), cp.Parameter(1)
            G_default, h_default = cp.Parameter((cut_size * 2 + variable_size, variable_size)), cp.Parameter(cut_size * 2 + variable_size)
    ...
    # ========================= Error message =========================
    if (torch.norm(T.detach() @ pred_optimal_coverage - T.detach() @ full_coverage_qp_solution) > 0.1):

...
if __name__ == '__main__':
    ...
    for epoch in range(-1, num_epochs):
        ...
        if training_method == 'surrogate':
            if epoch == -1:
                validate_loss, validate_obj = test_portfolio(model, covariance_model, epoch, validate_dataset, training_method=training_method, evaluate=True)
            else:
        ...
        # =============== printing data ================
        sys.stdout.write(f'Epoch {epoch} | Train Loss:    \t {train_loss:.3f} \t | Train Objective Value:    \t {train_obj*100:.3f}% \n')
        sys.stdout.write(f'Epoch {epoch} | Validate Loss: \t {validate_loss:.3f} \t | Validate Objective Value: \t {validate_obj*100:.3f}% \n')
        sys.stdout.write(f'Epoch {epoch} | Test Loss:     \t {test_loss:.3f} \t | Test Objective Value:     \t {test_obj*100:.3f}% \n')
        sys.stdout.flush()

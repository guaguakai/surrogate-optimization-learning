...
def get_optimal_coverage_prob(G, phi, U, initial_distribution, budget, omega=4, options={}):
    ...
    # coverage_prob_optimal= minimize(objective_function,initial_coverage_prob,args=(G, phi.detach().numpy(), U.numpy(), initial_distribution.numpy(), omega), method='SLSQP', bounds=bounds, constraints=constraints, options=options)
